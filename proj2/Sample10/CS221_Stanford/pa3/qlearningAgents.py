...
class QLearningAgent(ReinforcementAgent):
  ...
  def getPolicy(self, state):
    ...
    bestActions = [x for x in legalActions if self.getQValue(state, x) == max(qVals)]
...
class ApproximateQAgent(PacmanQAgent):
  ...
  def __init__(self, extractor='IdentityExtractor', **args):
    ...
    # You might want to initialize weights here."*** YOUR CODE HERE ***"
  ...
  def getQValue(self, state, action):
    ...
    for key in featureVector:
       	qVal += self.weight[key] * featureVector[key]
    return qVal
  ...
  def update(self, state, action, nextState, reward):
    ...
    featureVector = self.featExtractor.getFeatures(state, action)
    for key in featureVector:
      self.weight[key] += self.alpha * correction * featureVector[key]
