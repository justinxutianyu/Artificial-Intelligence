...
public class ASM {
    ...
    /** relation rule that corresponds to the subgraph */
	 ...
	 * @param subgraphWeights
	 */
	public ASM (RelationRule rule, AnnotatedSentence sentence,
			int subgraphDistanceThreshold, int[] subgraphWeights) {
		this.relationRule = rule;
		this.graph = sentence.getDependencyGraph().getGraph();
		this.subgraph = relationRule.getGraph();
		//set the startnode of subgraph
	    subgraphStartNode = getRandomStartNode( new ArrayList<Vertex>(subgraph.getVertices()) );
	    graphStartNodes = new ArrayList<Vertex>(graph.getVertices());
		...
		subgraphDistanceCache = relationRule.getDistanceCache();
		subgraphPathsCache = relationRule.getPathsCache();
		graphDistanceCache = new HashMap<Vertex, Map<Vertex, Integer>>();
		graphPathsCache = new HashMap<Vertex, Map<Vertex, List<List<Vertex>>>>();
	}
	...
	private boolean matchNodeContent(Vertex subgraphNode, Vertex graphNode) {
        if (graphNode.isArgument()) {
            LOG.info("Graph node is ARGUMENT");
        }
        else if (subgraphNode.isArgument()) {
            LOG.info("Subgraph node is ARGUMENT");
        }
        else if (subgraphNode.isArgument() && graphNode.isArgument()) {
            LOG.info("Both graph and subgraph nodes are ARGUMENTS");
        }
        else {
            LOG.info("Neither nodes are arguments");
        }
        LOG.info(
            "Comparing subgraph node {} to graph node {}",
            subgraphNode.getCompareForm(),
            graphNode.getCompareForm()
        );
