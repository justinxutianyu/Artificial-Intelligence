...
my @target_eclass = ('Gene_expression', 'Transcription', 'Protein_catabolism', 'Phosphorylation', 'Localization', 'Acetylation', 'Deacetylation', 'Protein_modification', 'Ubiquitination');
...
my (%protein, %gold, %gold_site, %equiv, %answer, %answer_site, %gold_type, %answer_type);
...
sub count_match {
    ...
    foreach my $type (keys %gold_type) {
            ...
            my @answer = (); foreach (keys %{$answer_type{$type}}) {if (/^[EM]/) {push @answer, $_; $cnt_manswer{$_} = 0}}
            my @gold   = (); foreach (keys %{$gold_type{$type}})   {if (/^[EM]/) {push @gold,   $_; $cnt_mgold{$_} = 0}}
            print "$#answer answers / $#gold gold to check\n";
...
sub read_a2_file ($$$) {
    ...
    my ($rh_anno, $rh_site, $rh_ranno, $rh_num_event, $rh_anno_type); # reference to hash
    if ($mode eq 'G') {$rh_anno = \%gold;   $rh_site = \%gold_site;   $rh_ranno = \%rgold;   $rh_num_event = \%num_gold; $rh_anno_type=\%gold_type}
    else              {$rh_anno = \%answer; $rh_site = \%answer_site; $rh_ranno = \%ranswer; $rh_num_event = \%num_answer; $rh_anno_type=\%answer_type}
	...
	$rh_anno_type->{$type}{$eid} = $rh_anno->{$eid};
    } # foreach
