...
# This file has only been tweaked to employ Procedural Content Generation
# for project purpose and rest of the code is the part of the framework 
...
def add_pacman_stuff(maze, max_food=MAX_FOOD, max_capsules=4, toskip=0):
  """
...
"""
generateMaze function tweaked to create random level each time
with variations in the depth of the prism and the the gaps in the maze
"""
def generateMaze(seed = 1):  
  cur_depth = random.randint(1,MAX_DEPTH)
  random.seed(seed)
  maze = Maze(16,16)
  gapfactor = min(0.65,random.gauss(0.5,0.1))
  # Varying the depth of the prism to test the robustness of the agent for making correct decisions.
  # Controlling the gap spaces to create the interesting game play.
  skip = make_with_prison(maze, depth=cur_depth, gaps=random.randint(1, 6), vert=True, min_width=1, gapfactor=gapfactor)
  maze.to_map()
