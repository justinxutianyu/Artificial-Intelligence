...
def readCommand( argv ):
  ...
  parser.add_option('--replaySpeed', default=0.05, type='float',
                    help=default('Speed in seconds per action'))
  ...
  if options.replay != None:
    ...
    recorded['replaySpeed'] = options.replaySpeed
    replayGame(**recorded)
    sys.exit(0)
...
def replayGame( layout, agents, actions, display, length, replaySpeed ):
    rules = CaptureRules()
